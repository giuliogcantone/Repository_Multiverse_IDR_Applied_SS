...
fread("multiverse.csv") |>
    filter(Cluster != "") |>
  summarise(
    Credibility = mean(p < 0.05 & Impact > 0),
    .by = c("Cluster","Indicator")
  ) |>
  pivot_wider(names_from = Cluster, values_from = Credibility, names_prefix = "Cred_") |>
  left_join(
    fread("multiverse.csv") |>
    filter(Cluster != "") |>
      summarise(
  ...
  left_join(
    ...
    filter(Cluster != "") |>
      summarise(
    pos = sum(Impact > 0 & p < .05)+1,
    neg = sum(Impact < 0 & p < .05)+1,
    Verac = log2(pos/neg),
    .by = c("Cluster","Indicator")) |>
  select(Indicator, Cluster, Verac) |>
  pivot_wider(names_from = Cluster, values_from = Verac,
              names_prefix = "Verac_")) |>
  left_join(
    fread("multiverse.csv") |>
    filter(Cluster != "") |>
...
bind_rows(
  fread("multiverse.csv") %>%
    filter(Cluster != "") %>%
    summarise(x = median(Impact, na.rm = TRUE), Cluster = "ALL", .by = Year) %>%
    rename(Element = Year) %>%
    mutate(Element = as.character(Element)),

  fread("multiverse.csv") %>%
    filter(Cluster != "") %>%
    summarise(x = median(Impact, na.rm = TRUE), Cluster = "ALL", .by = Lag) %>%
    rename(Element = Lag) %>%
    mutate(Element = as.character(Element)),

  fread("multiverse.csv") %>%
    filter(Cluster != "") %>%
    summarise(x = median(Impact, na.rm = TRUE), Cluster = "ALL", .by = FE) %>%
    rename(Element = FE) %>%
    mutate(Element = as.character(Element) %>% str_c("FE=",.)),

  fread("multiverse.csv") %>%
    filter(Cluster != "") %>%
    summarise(x = median(Impact, na.rm = TRUE), Cluster = "ALL", .by = N_author) %>%
    rename(Element = N_author) %>%
    mutate(Element = as.character(Element)  %>% str_c("Nauth=",.)),

  fread("multiverse.csv") %>%
    filter(Cluster != "") %>%
    summarise(x = median(Impact, na.rm = TRUE), Cluster = "ALL", .by = Stylisation) %>%
    rename(Element = Stylisation) %>%
    mutate(Element = as.character(Element)),

  fread("multiverse.csv") %>%
    filter(Cluster != "") %>%
    summarise(x = median(Impact, na.rm = TRUE), Cluster = "ALL", .by = Indicator) %>%
    rename(Element = Indicator) %>%
    mutate(Element = as.character(Element))
) |>
  add_row(
bind_rows(
fread("multiverse.csv") %>%
  summarise(x = median(Impact, na.rm = TRUE), .by = c(Year,Cluster)) |>
  dplyr::rename(Element = Year) |>
  mutate(Element = Element |> as.character()),
fread("multiverse.csv") %>%
  summarise(x = median(Impact, na.rm = TRUE), .by = c(Lag,Cluster)) |>
  dplyr::rename(Element = Lag),
fread("multiverse.csv") %>%
  summarise(x = median(Impact, na.rm = TRUE), .by = c(FE,Cluster)) |>
  dplyr::rename(Element = FE) |>
  mutate(Element = Element |> as.character() %>% str_c("FE=",.)),
fread("multiverse.csv") %>%
  summarise(x = median(Impact, na.rm = TRUE), .by = c(N_author,Cluster)) |>
  dplyr::rename(Element = N_author) |>
  mutate(Element = Element |> as.character() %>% str_c("Nauth=",.)),
fread("multiverse.csv") %>%
  summarise(x = median(Impact, na.rm = TRUE), .by = c(Stylisation,Cluster)) |>
  dplyr::rename(Element = Stylisation) |>
  mutate(Element = Element |> as.character()),
fread("multiverse.csv") %>%
  summarise(x = median(Impact, na.rm = TRUE), .by = c(Indicator,Cluster)) |>
  dplyr::rename(Element = Indicator) |>
  mutate(Element = Element |> as.character())
)) |>
  mutate(
    Cluster = ifelse(Cluster == "","NO",Cluster),
    x = x |> round(2)
  ) |>
  filter(Cluster != "NO") |>
  pivot_wider(
  ...
  select(Element,MGMT,FINAC,MKTG, ALL) |>
  janitor::adorn_rounding(2) |>
       knitr::kable(
